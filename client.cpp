...
int client_on_timer(conn_info_t& conn_info) // for client. called when a timer is ready in epoll
{
    ...
    mylog(log_trace, "roller my %d,oppsite %d,%lld\n", int(conn_info.my_roller),
          int(conn_info.oppsite_roller), conn_info.last_oppsite_roller_time);
    ...
    if (conn_info.state.client_current_state == client_idle)
    {
        ...
        if (raw_mode == mode_faketcp)
        {
            if (use_tcp_dummy_socket)
            {
                ...
                mylog(log_info,
                      "state changed from client_idle to "
    ...
    else if (conn_info.state.client_current_state == client_ready)
    {
        ...
        if (get_current_time() - conn_info.last_hb_recv_time > client_conn_timeout)
        {
            ...
            conn_info.my_id = get_true_random_number_nz();
            mylog(log_info,
                  "state back to client_idle from  client_ready bc of "
...
int client_on_raw_recv(conn_info_t& conn_info) // called when raw fd received a packet.
{
    ...
    if (conn_info.state.client_current_state == client_idle)
    {
        ...
        // recv(raw_recv_fd, 0,0, 0  );
             ...
             conn_info.state.client_current_state == client_tcp_handshake_dummy) // received syn ack
    {
        assert(raw_mode == mode_faketcp);
        ...
        if (!recv_info.new_src_ip.equal(send_info.new_dst_ip) || recv_info.src_port != send_info.dst_port)
        {
            mylog(log_debug, "unexpected adress %s %s %d %d\n", recv_info.new_src_ip.get_str1(),
                  send_info.new_dst_ip.get_str2(), recv_info.src_port, send_info.dst_port);
            return -1;
        }
        if (data_len == 0 && raw_info.recv_info.syn == 1 && raw_info.recv_info.ack == 1)
        {
            if (conn_info.state.client_current_state == client_tcp_handshake)
            {
                ...
                mylog(log_info,
                      "state changed from client_tcp_handshake to "
                      "client_handshake1\n");
            }
            else
            {
                ...
                mylog(log_info,
                      "state changed from client_tcp_dummy to "
                      "client_handshake1\n");
                // send_info.ack_seq=recv_info.seq+1;
            }
            conn_info.state.client_current_state = client_handshake1;
    ...
    else if (conn_info.state.client_current_state == client_handshake1) // recevied respond of handshake1
    {
        ...
        mylog(log_info,
              "changed state from to client_handshake1 to "
              "client_handshake2,my_id is %x,oppsite id "
              "is %x\n",
        ...
        return 0;
             ...
             conn_info.state.client_current_state == client_ready) // received heartbeat or data
    {
        char type;
        ...
        else if (data_len >= int(sizeof(u32_t)) && type == 'd')
        {
            ...
            int ret = sendto(udp_fd, data + sizeof(u32_t), data_len - (sizeof(u32_t)), 0,
                             (struct sockaddr*)&tmp_addr.inner, tmp_addr.get_len());

            if (ret < 0)
            {
                mylog(log_warn, "sento returned %d,%s,%02x,%s\n", ret, get_sock_error(),
                      int(tmp_addr.get_type()), tmp_addr.get_str());
                // perror("ret<0");
            }
        }
        else
...
int client_on_udp_recv(conn_info_t& conn_info)
{
    ...
    if ((recv_len = recvfrom(udp_fd, buf, max_data_len + 1, 0, (struct sockaddr*)&udp_new_addr_in,
                             &udp_new_addr_len)) == -1)
    {
...
int client_event_loop()
{
    ...
    if (lower_level)
    {
        ...
        else
        {
            ...
            if (retry_on_error == 0)
            {
                if (find_lower_level_info(remote_addr.inner.ipv4.sin_addr.s_addr, dest_ip,
                                          if_name_string, hw_string) != 0)
                {
                    mylog(log_fatal,
                          "auto detect lower-level info failed for %s,specific "
                          "it manually\n",
                          remote_addr.get_ip());
                    myexit(-1);
                }
            ...
            else
            {
                ...
                while (!ok)
                {
                    ...
                    {
                        mylog(log_warn,
                              "auto detect lower-level info failed for "
            ...
            sscanf(hw_string.c_str(), "%x:%x:%x:%x:%x:%x", &hw[0], &hw[1], &hw[2], &hw[3], &hw[4], &hw[5]);
