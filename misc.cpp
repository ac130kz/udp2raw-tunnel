...
// char local_ip[100]="0.0.0.0",
// remote_ip[100]="255.255.255.255",source_ip[100]="0.0.0.0";//local_ip is for -l option,remote_ip
// for -r option,source for --source-ip u32_t
// local_ip_uint32,remote_ip_uint32,source_ip_uint32;//convert from last line. int local_port = -1,
// remote_port=-1,source_port=0;//similiar to local_ip  remote_ip,buf for port.source_port=0
// indicates --source-port is not enabled
...
my_id_t const_id = 0; // an id used for connection recovery,its generated randomly,it never change
                      // since its generated

int udp_fd = -1;   // for client only. client use this fd to listen and handle udp connection
int bind_fd = -1;  // bind only,never send or recv.  its just a dummy fd for bind,so that other
                   // program wont occupy the same port
int epollfd = -1;  // fd for epoll
int timer_fd = -1; // the general timer fd for client and server.for server this is not the only
                   // timer find,every connection has a timer fd.
...
int process_lower_level_arg() // handle --lower-level option
{
    ...
    if (strchr(optarg, '#') == 0)
    {
        mylog(log_fatal, "lower-level parameter invaild,check help page for format\n");
        myexit(-1);
    }
    lower_level = 1;
    u32_t hw[6];
    ...
    mylog(log_warn,
          "make sure this is correct:   if_name=<%s>  "
          "dest_mac_adress=<%02x:%02x:%02x:%02x:%02x:%02x>  \n",
...
void print_help()
{
    ...
    printf("usage:\n");
    printf(
        "    run as client : ./this_program -c -l local_listen_ip:local_port -r "
        "server_address:server_port  [options]\n");
    printf(
        "    run as server : ./this_program -s -l server_listen_ip:server_port -r "
        "remote_address:remote_port  [options]\n");
    printf("\n");
    printf("common options,these options must be same on both side:\n");
    printf(
        "    --raw-mode            <string>        avaliable values:faketcp(default),udp,icmp\n");
    printf(
        "    -k,--key              <string>        password to gen symetric key,default:\"secret "
        "key\"\n");
    printf(
        "    --cipher-mode         <string>        avaliable "
        "values:aes128cfb,aes128cbc(default),xor,none\n");
    printf(
        "    --auth-mode           <string>        avaliable "
        "values:hmac_sha1,md5(default),crc32,simple,none\n");
    printf("    -a,--auto-rule                        auto add (and delete) iptables rule\n");
    printf(
        "    -g,--gen-rule                         generate iptables rule then exit,so that you "
        "can copy and\n");
    ...
    printf(
        "    --source-port         <port>          force source-port for raw socket,tcp/udp "
        "only\n");
    printf(
        "                                          this option disables port changing while "
        "re-connecting\n");
    //	printf("                                          \n");
    printf("other options:\n");
    printf(
        "    --conf-file           <string>        read options from a configuration file instead "
        "of command line.\n");
    printf("                                          check example.conf in repo for format\n");
    printf(
        "    --fifo                <string>        use a fifo(named pipe) for sending commands to "
        "the running program,\n");
    printf(
        "                                          check readme.md in repository for supported "
        "commands.\n");
    printf("    --log-level           <number>        0:never    1:fatal   2:error   3:warn \n");
    printf("                                          4:info (default)     5:debug   6:trace\n");
    //	printf("\n");
    printf(
        "    --log-position                        enable file name,function name,line number in "
        "log\n");
    printf("    --disable-color                       disable log color\n");
    printf(
        "    --disable-bpf                         disable the kernel space filter,most time its "
        "not necessary\n");
    printf("                                          unless you suspect there is a bug\n");
    //	printf("\n");
    printf(
        "    --dev                 <string>        bind raw socket to a device, not necessary but "
        "improves performance\n");
    printf(
        "    --sock-buf            <number>        buf size for socket,>=10 and "
        "<=10240,unit:kbyte,default:1024\n");
    printf(
        "    --force-sock-buf                      bypass system limitation while setting "
        "sock-buf\n");
    printf("    --seq-mode            <number>        seq increase mode for faketcp:\n");
    printf(
        "                                          0:static header,do not increase seq and "
        "ack_seq\n");
    printf(
        "                                          1:increase seq for every packet,simply ack last "
        "seq\n");
    printf(
        "                                          2:increase seq randomly, about every 3 "
        "packets,simply ack last seq\n");
    printf(
        "                                          3:simulate an almost real seq/ack "
        "procedure(default)\n");
    printf(
        "                                          4:similiar to 3,but do not consider TCP Option "
        "Window_Scale,\n");
    printf(
        "                                          maybe useful when firewall doesnt support TCP "
        "Option \n");
    //	printf("\n");
    printf(
        "    --lower-level         <string>        send packets at OSI level 2, "
        "format:'if_name#dest_mac_adress'\n");
    printf(
        "                                          ie:'eth0#00:23:45:67:89:b9'.or try "
        "'--lower-level auto' to obtain\n");
    printf(
        "                                          the parameter automatically,specify it manually "
        "if 'auto' failed\n");
    printf(
        "    --wait-lock                           wait for xtables lock while invoking iptables, "
        "need iptables v1.4.20+\n");
    printf(
        "    --gen-add                             generate iptables rule and add it "
        "permanently,then exit.overrides -g\n");
    printf(
        "    --keep-rule                           monitor iptables and auto re-add if "
        "necessary.implys -a\n");
    printf(
        "    --hb-len              <number>        length of heart-beat packet, >=0 and <=1500\n");
    printf(
        "    --mtu-warn            <number>        mtu warning threshold, unit:byte, "
        "default:1375\n");
    printf(
        "    --clear                               clear any iptables rules added by this "
        "program.overrides everything\n");
    printf(
        "    --retry-on-error                      retry on error, allow to start udp2raw before "
...
int load_config(char* file_name,
                int& argc,
                vector<string>& argv) // load conf file and append to argv
{
...
void process_arg(int argc, char* argv[]) // process all options
{
    ...
    while ((opt = getopt_long(argc, argv, options, long_options, &option_index)) != -1)
    {
        ...
        case 1:
            ...
            else if (strcmp(long_options[option_index].name, "raw-mode") == 0)
            {
                 ...
                 */
                if (strcmp(optarg, "easyfaketcp") == 0 || strcmp(optarg, "easy_faketcp") == 0 ||
                    strcmp(optarg, "easy-faketcp") == 0)
                {
            ...
            else if (strcmp(long_options[option_index].name, "easy-tcp") == 0)
            {
                use_tcp_dummy_socket = 1;
                mylog(log_info,
                      "--easy-tcp enabled, now a dummy tcp socket will be created for "
                      "handshake and block rst\n");
            }
            else
            {
    ...
    {
        mylog(log_error,
              "-a,--auto-rule is not supposed to be used with easyfaketcp mode, you are likely "
              "making a mistake, but we can try to continue\n");
    }

    if (keep_rule && use_tcp_dummy_socket)
    {
        mylog(log_error,
              "--keep-rule is not supposed to be used with easyfaketcp mode, you are likely making "
              "a mistake, but we can try to continue\n");
    }
...
void iptables_rule() // handles -a -g --gen-add  --keep-rule --clear --wait-lock
{
    ...
    {
        char* output;
        // int ret =system("iptables-save |grep udp2raw_dWRwMnJhdw|sed -n 's/^-A/iptables
        // -D/p'|sh");
        int ret = run_command(iptables_command + "-S|sed -n '/udp2rawDwrW/p'|sed -n 's/^-A/" +
                                  iptables_command + "-D/p'|sh",
                              output);

        int ret2 = run_command(iptables_command + "-S|sed -n '/udp2rawDwrW/p'|sed -n 's/^-N/" +
                                   iptables_command + "-X/p'|sh",
                               output);
    ...
    else
    {
        mylog(
            log_warn,
            " -a has not been set, make sure you have added the needed iptables rules manually\n");
    }
}
...
int unit_test()
{
    ...
    vector<string> conf_lines = {"---aaa", "--aaa bbb", "-a bbb",
                                 " \t \t \t-a\t \t \t bbbbb\t \t \t "};
...
int set_timer(int epollfd,
              int& timer_fd) // put a timer_fd into epoll,general function,used both in client and server
{
    int ret;
    epoll_event ev;
...
int handle_lower_level(raw_info_t& raw_info) // fill lower_level info,when --lower-level is enabled,only for server
{
    ...
    if (lower_level_manual)
    {
        ...
        mylog(log_debug, "[manual]lower level info %x %x\n ", send_info.addr_ll.sll_halen,
              send_info.addr_ll.sll_protocol);
    }
    else
    {
        memset(&send_info.addr_ll, 0, sizeof(send_info.addr_ll));
...
int keep_iptables_rule() // magic to work on a machine without grep/iptables --check/-m commment
{
    ...
    run_command(rule_keep_del[i], output,
                show_none); // do it twice,incase it fails for unknown random reason
