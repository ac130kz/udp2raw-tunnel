...
unsigned char dest_hw_addr[sizeof(sockaddr_ll::sll_addr)] = {0xff, 0xff, 0xff, 0xff,
                                                             0xff, 0xff, 0,    0};
...
struct sock_filter code_udp[] = {
    //{ 0x5, 0, 0, 0x00000001 },
    //{ 0x5, 0, 0, 0x00000000 },
    {0x30, 0, 0, 0x00000009}, {0x15, 0, 6, 0x00000011}, {0x28, 0, 0, 0x00000006},
    {0x45, 4, 0, 0x00001fff}, {0xb1, 0, 0, 0x00000000}, {0x48, 0, 0, 0x00000002},
...
struct sock_filter code_udp6[] = {
    //		{ 0x28, 0, 0, 0x0000000c },
    //		{ 0x15, 0, 5, 0x000086dd },
    {0x30, 0, 0, 0x00000006}, {0x15, 0, 3, 0x00000011}, {0x28, 0, 0, 0x0000002a},
    {0x15, 0, 1, 0x0000fffe}, {0x6, 0, 0, 0x00040000},  {0x6, 0, 0, 0x00000000},
...
struct sock_filter code_icmp6[] = {
    //		{ 0x28, 0, 0, 0x0000000c },
    //		{ 0x15, 0, 6, 0x000086dd },
    {0x30, 0, 0, 0x00000006}, {0x15, 3, 0, 0x0000003a}, {0x15, 0, 3, 0x0000002c},
    {0x30, 0, 0, 0x00000028}, {0x15, 0, 1, 0x0000003a}, {0x6, 0, 0, 0x00040000},
    {0x6, 0, 0, 0x00000000},
...
int init_raw_socket()
{
    ...
    {
        raw_send_fd = socket(PF_PACKET, SOCK_DGRAM,
                             htons(ETH_P_IP)); // todo  how to create a recv only raw socket?
    ...
    int opt = 0;
    assert(setsockopt(raw_send_fd, SOL_SOCKET, SO_RCVBUF, &opt, sizeof(opt)) ==
           0); // raw_send_fd is for send only, set its recv buffer to zero
...
void init_filter(int port)
{
    ...
    int ret = setsockopt(raw_recv_fd, SOL_SOCKET, SO_DETACH_FILTER, &dummy,
                         sizeof(dummy)); // in case i forgot to remove
    if (ret != 0)
    {
...
int find_arp(const vector<arp_info_t>& arp_info_vec, u32_t ip, string if_name, string& hw)
{
    ...
    if (count == 0)
    {
        // mylog(log_warn,"cant find arp entry for %s %s,using
        // 00:00:00:00:00:00\n",my_ntoa(ip),if_name.c_str()); hw="00:00:00:00:00:00";
...
int find_lower_level_info(u32_t ip, u32_t& dest_ip, string& if_name, string& hw)
{
    ...
    string route_file;
    if (read_file("/proc/net/route", route_file) != 0)
        return -1;
    string arp_file;
    if (read_file("/proc/net/arp", arp_file) != 0)
        return -1;
    ...
    for (u32_t i = 1; i < route_vec2.size(); i++)
    {
        ...
        if (hex_to_u32_with_endian(route_vec2[i][dest_idx], tmp.dest) != 0)
            return -1;
        if (hex_to_u32_with_endian(route_vec2[i][gw_idx], tmp.gw) != 0)
            return -1;
        if (hex_to_u32_with_endian(route_vec2[i][mask_idx], tmp.mask) != 0)
            return -1;
        if (hex_to_u32(route_vec2[i][flag_idx], tmp.flag) != 0)
            return -1;
        ...
        log_bare(log_debug, "%s dest:%x mask:%x gw:%x flag:%x", tmp.if_name.c_str(), tmp.dest,
                 tmp.mask, tmp.gw, tmp.flag);
        log_bare(log_debug, "\n");
    }
...
int send_raw_ip(raw_info_t& raw_info, const char* payload, int payloadlen)
{
    ...
    if (raw_ip_version == AF_INET)
    {
        ...
        {
            iph->id = htons(g_ip_id_counter++); // Id of this packet
            // iph->id = 0; //Id of this packet  ,kernel will auto fill this if id is zero  ,or
            // really?????// todo //seems like there is a problem
        }
        ...
        if (lower_level)
            iph->tot_len = htons(ip_tot_len); // this is not necessary ,kernel will always auto fill this
        ...
        if (lower_level)
            iph->check = csum((unsigned short*)send_raw_ip_buf,
                              iph->ihl * 4); // this is not necessary ,kernel will always auto fill this
        else
            iph->check = 0;
    }
    else
...
int pre_recv_raw_packet()
{
    ...
    // assert(g_sockaddr_len==sizeof(g_sockaddr.ll)); //g_sockaddr_len=18, sizeof(g_sockaddr.ll)=20,
    // why its not equal? maybe its bc sll_halen is 6?
...
int recv_raw_ip(raw_info_t& raw_info, char*& payload, int& payloadlen)
{
    ...
    if (recv_raw_packet(raw_packet_buf, raw_packet_len, raw_info.peek) != 0)
        return -1;
    ...
    my_ip6hdr* ip6h;
    int flag = 0;
    // int recv_len = recvfrom(raw_recv_fd, recv_raw_ip_buf, max_data_len+1, flag ,(sockaddr*)&saddr
    // , &saddr_size);
...
int peek_raw(raw_info_t& raw_info)
{
    ...
    // int recv_len = recvfrom(raw_recv_fd, peek_raw_buf,max_data_len, MSG_PEEK ,&saddr ,
    // &saddr_size);//change max_data_len to something smaller,we only need header here iphdr * iph
    // = (struct iphdr *) (ip_begin); mylog(log_info,"recv_len %d\n",recv_len);
...
int send_raw_icmp(raw_info_t& raw_info, const char* payload, int payloadlen)
{
    ...
    else
    {
        ...
        icmph->check_sum = csum_with_header((char*)psh, sizeof(pseudo_header6), (unsigned short*)send_raw_icmp_buf,
                                            sizeof(my_icmphdr) + payloadlen);
    }
...
int send_raw_udp(raw_info_t& raw_info, const char* payload, int payloadlen)
{
    ...
    if (raw_ip_version == AF_INET)
    {
        ...
        udph->check = csum_with_header((char*)psh, sizeof(pseudo_header),
                                       (unsigned short*)send_raw_udp_buf, udp_tot_len);
    }
    else
    {
        assert(raw_ip_version == AF_INET6);
        ...
        udph->check = csum_with_header((char*)psh, sizeof(pseudo_header6),
                                       (unsigned short*)send_raw_udp_buf, udp_tot_len);
    }
...
{ // TODO seq increase
    ...
    tcph->window =
        htons((uint16_t)(receive_window_lower_bound + get_true_random_number() % receive_window_random_range));
    ...
    if (raw_ip_version == AF_INET)
    {
        ...
        tcph->check = csum_with_header((char*)psh, sizeof(pseudo_header),
                                       (unsigned short*)send_raw_tcp_buf, tcp_totlen);
    }
    else
    {
        assert(raw_ip_version == AF_INET6);
        ...
        tcph->check = csum_with_header((char*)psh, sizeof(pseudo_header6),
                                       (unsigned short*)send_raw_tcp_buf, tcp_totlen);
    }
...
int send_raw_tcp_deprecated(const packet_info_t &info,const char * payload,int payloadlen)
{
    ...
    mylog(log_trace,"send raw from to %d %d %d
%d\n",info.src_ip,info.src_port,info.dst_ip,info.dst_port);
    ...
    int psize = sizeof(struct pseudo_header) + tcph->doff*4 + payloadlen;
     ...
     mylog(log_trace,"sent seq  ack_seq len<%u %u
%d>\n",g_packet_info_send.seq,g_packet_info_send.ack_seq,payloadlen);

     int ret = sendto(raw_send_fd, raw_send_buf, iph->tot_len ,  0, (struct sockaddr *) &sin, sizeof
(sin));
...
int recv_raw_udp(raw_info_t& raw_info, char*& payload, int& payloadlen)
{
    ...
    uint16_t udp_chk = csum_with_header((char *)psh,sizeof(pseudo_header), (unsigned short*)
    ip_payload, csum_len);
    */
    uint16_t udp_chk;
    int csum_len = ip_payloadlen;
    ...
    */
    ...
    if (tcph->rst == 1)
    {
        ...
        if (max_rst_to_show > 0)
        {
            ...
            {
                mylog(log_warn, "[%s,%d]rst==1,cnt=%d\n", recv_info.new_src_ip.get_str1(),
                      recv_info.src_port, (int)raw_info.rst_received);
            }
            else if (raw_info.rst_received == max_rst_to_show)
            {
                mylog(log_warn,
                      "[%s,%d]rst==1,cnt=%d >=max_rst_to_show, this log will be muted for current "
                      "connection\n",
        ...
        {
            mylog(log_warn,
                  "[%s,%d]connection disabled because of rst_received=%d > max_rst_allow=%d\n",
                  recv_info.new_src_ip.get_str1(), recv_info.src_port, (int)raw_info.rst_received,
                  (int)max_rst_allowed);
            raw_info.disabled = 1;
        }
    ...
    /*if (recv_info.syn == 0 && recv_info.ack == 1&& payloadlen != 0)   //only modify   send_info
    when the packet is not part of handshake
    {
...
int recv_raw_tcp_deprecated(packet_info_t &info,char * &payload,int &payloadlen)
{
   ...
   if(ip_chk!=0)
    ...
    mylog(log_trace,"<%u,%u,%u,%u,%d>\n",(unsigned int)iphdrlen,(unsigned int)tcphdrlen,(unsigned
int)tcph->syn,(unsigned int)tcph->ack,payloadlen);
...
int send_raw0(raw_info_t& raw_info, const char* payload, int payloadlen)
{
    ...
    packet_info_t& recv_info = raw_info.recv_info;
    mylog(log_trace, "send_raw : from %s %d  to %s %d\n", send_info.new_src_ip.get_str1(),
          send_info.src_port, send_info.new_dst_ip.get_str2(), send_info.dst_port);
    switch (raw_mode)
    {
    case mode_faketcp:
...
int after_send_raw0(raw_info_t& raw_info)
{
    ...
    if (raw_mode == mode_faketcp)
    {
        if (send_info.syn == 0 && send_info.ack == 1 &&
            raw_info.send_info.data_len != 0) // only modify   send_info when the packet is not part of handshake
        {
            if (seq_mode == 0)
            {
            }
            ...
            else if (seq_mode == 3 || seq_mode == 4)
            {
                ...
                if (larger_than_u32(raw_info.recv_info.ack_seq,
                                    send_info.seq)) // for further use,currently no effect.
                {
                    send_info.seq = raw_info.recv_info.ack_seq;
                }
            }
...
int after_recv_raw0(raw_info_t& raw_info)
{
    ...
    if (raw_mode == mode_faketcp)
    {
        ...
        if (recv_info.syn == 0 && recv_info.ack == 1 &&
            raw_info.recv_info.data_len != 0) // only modify   send_info when the packet is not part of handshake
        {
            ...
            else if (seq_mode == 3 || seq_mode == 4)
            {
                ...
                {
                    send_info.ack_seq =
                        recv_info.seq + raw_info.recv_info.data_len; // currently we dont remembr tcp segments,this
                                                                     // is the simplest way
...
/*
int get_src_adress(u32_t &ip,u32_t remote_ip_uint32,int remote_port)  //a trick to get src adress
for a dest adress,so that we can use the src address in raw socket as source ip
{
    ...
    if(getsockname(new_udp_fd, (struct sockaddr *) &my_addr, &len)!=0){close(new_udp_fd); return
-1;}
...
int client_bind_to_a_new_port2(int& fd,
                               const address_t& address) // find a free port and bind to it.
{
