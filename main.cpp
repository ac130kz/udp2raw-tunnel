...
int main(int argc, char* argv[])
{
    ...
    {
        mylog(log_warn,
              "root check failed, it seems like you are using a non-root "
              "account. we can try to "
              "continue, but it may fail. If you want to run udp2raw as "
              "non-root, you have to add "
              "iptables rule manually, and grant udp2raw CAP_NET_RAW "
              "capability, check README.md "
              "in repo for more info.\n");
    }
    else
    {
        mylog(log_warn,
              "you can run udp2raw with non-root account for better security. "
              "check README.md in "
              "repo for more info.\n");
    }
#endif
