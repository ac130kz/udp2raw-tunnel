...
const int disable_conv_clear = 0; // a udp connection in the multiplexer is called conversation in
                                  // this program,conv for short.
...
struct conn_info_t // stores info for a raw connection.for client ,there is only one connection,for
                   // server there can be thousand of connection since server can
...
struct conn_manager_t // manager for connections. for client,we dont need conn_manager since there
                      // is only one connection.for server we use one conn_manager for all
                      // connections
{
    u32_t ready_num;

    // unordered_map<int,conn_info_t *> udp_fd_mp;  //a bit dirty to used pointer,but can void
    // unordered_map search unordered_map<int,conn_info_t *> timer_fd_mp;//we can use pointer here
    // since unordered_map.rehash() uses shallow copy
...
int send_bare(raw_info_t& raw_info,
              const char* data,
              int len); // send function with encryption but no anti replay,this is used when
                        // client and server verifys each other
// you have to design the protocol carefully, so that you wont be affect by relay attack
// int reserved_parse_bare(const char *input,int input_len,char* & data,int & len); // a sub
// function used in recv_bare
int recv_bare(raw_info_t& raw_info,
              char*& data,
              int& len); // recv function with encryption but no anti replay,this is used when
                         // client and server verifys each other
// you have to design the protocol carefully, so that you wont be affect by relay attack
int send_handshake(raw_info_t& raw_info,
                   my_id_t id1,
                   my_id_t id2,
                   my_id_t id3); // a warp for send_bare for sending handshake(this is not tcp handshake) easily
int send_safer(conn_info_t& conn_info,
               char type,
               const char* data,
               int len); // safer transfer function with anti-replay,when mutually verification is done.
int send_data_safer(conn_info_t& conn_info,
                    const char* data,
                    int len,
                    u32_t conv_num); // a wrap for  send_safer for transfer data.
// int reserved_parse_safer(conn_info_t &conn_info,const char * input,int input_len,char &type,char*
// &data,int &len);//subfunction for recv_safer,allow overlap
int recv_safer(conn_info_t& conn_info,
               char& type,
               char*& data,
               int& len); /// safer transfer function with anti-replay,when mutually verification is done.
