...
int server_on_timer_multi(conn_info_t& conn_info) // for server. called when a timer is ready in epoll.for server,there
                                                  // will be one timer for every connection
// there is also a global timer for server,but its not handled here
{
    ...
    address_t tmp_addr;
    tmp_addr.from_ip_port_new(raw_ip_version, &conn_info.raw_info.send_info.new_dst_ip,
                              conn_info.raw_info.send_info.dst_port);
...
int server_on_raw_recv_ready(conn_info_t& conn_info,
                             ...
                             char type,
                             char* data,
                             int data_len) // called while the state for a connection is server_ready
// receives data and heart beat by recv_safer.
{
    ...
    else if (type == 'd' && data_len >= int(sizeof(u32_t)))
    {
        ...
        if (!conn_info.blob->conv_manager.s.is_conv_used(tmp_conv_id))
        {
            ...
            {
                mylog(log_warn, "[%s]ignored new conv %x connect bc max_conv_num exceed\n", ip_port, tmp_conv_id);
                return 0;
            }
        ...
        int ret = send(fd, data + sizeof(u32_t), data_len - (sizeof(u32_t)), 0);
...
int server_on_raw_recv_pre_ready(conn_info_t& conn_info,
                                 char* ip_port,
                                 u32_t tmp_oppsite_const_id) // do prepare work before state change to server ready for a
                                                             // specifc connection
// connection recovery is also handle here
{
    ...
    else
    {
        ...
        if (ori_conn_info.state.server_current_state == server_ready)
        {
            ...
            {
                mylog(log_info,
                      "[%s]conn_info.last_state_time<ori_conn_info.last_state_"
                      "time. ignored new "
                      "handshake\n",
                      ip_port);
                conn_info.state.server_current_state = server_idle;
            ...
            address_t addr1;
            addr1.from_ip_port_new(raw_ip_version, &ori_conn_info.raw_info.recv_info.new_src_ip,
                                   ori_conn_info.raw_info.recv_info.src_port);
            ...
            address_t addr2;
            addr2.from_ip_port_new(raw_ip_version, &conn_info.raw_info.recv_info.new_src_ip,
                                   conn_info.raw_info.recv_info.src_port);
...
int server_on_raw_recv_handshake1(conn_info_t& conn_info,
                                  char* ip_port,
                                  char* data,
                                  int data_len) // called when server received a handshake1 packet from client
{
    packet_info_t& send_info = conn_info.raw_info.send_info;
    packet_info_t& recv_info = conn_info.raw_info.recv_info;
    raw_info_t& raw_info = conn_info.raw_info;
...
int server_on_udp_recv(conn_info_t& conn_info, fd64_t fd64)
{
    ...
    {
        mylog(log_fatal,
              "p_conn_info->state.server_current_state!=server_ready!!!this "
              "shouldnt happen\n");
        myexit(-1);
    }
    ...
    {
        mylog(log_warn,
              "huge packet,data len=%d (>=%d).strongly suggested to set a "
              "smaller mtu at upper "
              "level,to get rid of this warn\n ",
              recv_len, mtu_warn);
    }

    // conn_info.conv_manager->update_active_time(conv_id);  server dosnt update
    // from upd side,only update from raw side.  (client updates at both side)
...
int server_event_loop()
{
    ...
    else if (raw_mode == mode_udp || raw_mode == mode_icmp) // bind an adress to avoid collision,for icmp,there is no
                                                            // port,just bind a udp port
    {
