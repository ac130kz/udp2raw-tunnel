...
conn_info_t::~conn_info_t()
{
    if (program_mode == server_mode)
    {
        if (state.server_current_state == server_ready)
        {
            ...
            // assert(conn_manager.const_id_mp.find(oppsite_const_id)!=conn_manager.const_id_mp.end());
            // // conn_manager 's deconstuction function  erases it
        }
        else
        {
            assert(blob == 0);
...
int conn_manager_t::clear_inactive0()
{
    ...
    for (;;)
    {
        ...
        if (it->second->state.server_current_state == server_ready &&
            current_time - it->second->last_hb_recv_time <= server_conn_timeout)
        {
            it++;
        }
        else if (it->second->state.server_current_state != server_ready &&
                 current_time - it->second->last_state_time <= server_handshake_timeout)
        {
            it++;
        }
        ...
        else
        {
            mylog(log_info, "[%s:%d]inactive conn cleared \n",
                  it->second->raw_info.recv_info.new_src_ip.get_str1(),
                  it->second->raw_info.recv_info.src_port);
            old_it = it;
            it++;
            erase(old_it);
        }
...
int send_bare(raw_info_t& raw_info,
              const char* data,
              int len) // send function with encryption but no anti replay,this is used when client
                       // and server verifys each other
...
int reserved_parse_bare(const char* input,
                        int input_len,
                        char*& data,
                        int& len) // a sub function used in recv_bare
{
                   ...
                   my_id_t id3) // a warp for send_bare for sending handshake(this is not tcp handshake) easily
{
    ...
    int len;
    // len=sizeof(id_t)*3;
    if (numbers_to_char(id1, id2, id3, data, len) != 0)
        return -1;
               ...
               int len) // safer transfer function with anti-replay,when mutually verification is done.
{
    ...
    if (send_raw0(conn_info.raw_info, send_data_buf2, new_len) != 0)
        return -1;

    if (after_send_raw0(conn_info.raw_info) != 0)
        return -1;
...
int reserved_parse_safer(conn_info_t& conn_info, const char* input, int input_len, char& type, char*& data, int& len) // subfunction for recv_safer,allow overlap
{
    ...
    if (h_oppsite_id != conn_info.oppsite_id || h_my_id != conn_info.my_id)
    {
        mylog(log_debug, "id and oppsite_id verification failed %x %x %x %x \n", h_oppsite_id,
              conn_info.oppsite_id, h_my_id, conn_info.my_id);
        return -1;
    }
               ...
               int& len) /// safer transfer function with anti-replay,when mutually verification is done.
{
    ...
    if (recv_raw0(conn_info.raw_info, recv_data, recv_len) != 0)
        return -1;

    return reserved_parse_safer(conn_info, recv_data, recv_len, type, data, len);
}

void server_clear_function(u64_t u64) // used in conv_manager in server mode.for server we have to
                                      // use one udp fd for one conv(udp connection),
