...
const u32_t server_handshake_timeout =
    client_handshake_timeout + 5000; // this should be longer than clients. client retry initially ,server retry passtively
...
const uint32_t server_conn_timeout =
    conv_timeout + 60000; // ms. this should be 60s+ longer than conv_timeout,so that conv_manager
                          // can destruct convs gradually,to avoid latency glicth
// const u32_t server_conn_timeout=conv_timeout+10000;//for test

const u32_t iptables_rule_keep_interval = 20; // unit: second;

enum server_current_state_t
{
    server_idle = 0,
    server_handshake1,
    server_ready
}; // server state machine
enum client_current_state_t
{
    client_idle = 0,
...
// extern char local_ip[100], remote_ip[100],source_ip[100];//local_ip is for -l option,remote_ip
// for -r option,source for --source-ip extern u32_t
// local_ip_uint32,remote_ip_uint32,source_ip_uint32;//convert from last line. extern int local_port
// , remote_port,source_port;//similiar to local_ip  remote_ip,buf for port.source_port=0 indicates
// --source-port is not enabled
...
extern my_id_t const_id; // an id used for connection recovery,its generated randomly,it never
                         // change since its generated

extern int udp_fd;   // for client only. client use this fd to listen and handle udp connection
extern int bind_fd;  // bind only,never send or recv.  its just a dummy fd for bind,so that other
                     // program wont occupy the same port
extern int epollfd;  // fd for epoll
extern int timer_fd; // the general timer fd for client and server.for server this is not the only
